...
### Example 1: Don't Accept the Proposed Fix Blindly

**User Request:** "Add a database index to fix the slow order search"

**❌ What LLMs Do (Accept the Diagnosis Too Quickly)**

```python
# Adds an index immediately because the user said the database is slow
op.create_index(
...
**Problems:**
- Assumed the database is the bottleneck
- Didn't inspect the actual slow path
- Didn't verify whether the issue is query time, N+1 queries, or response construction
- Implemented a fix before confirming the cause

**✅ What Should Happen (Pressure-test the Premise)**

```
Before changing schema, verify that indexing is the right fix:

1. Measure where the latency comes from:
   - database query time
   - application processing time
   - serialization / response size
2. Inspect the actual query and run EXPLAIN
3. Check whether the endpoint is slow for all searches or only specific filters
4. Confirm whether the issue is a missing index, N+1 queries, or too much returned data

A safer response is:

"I wouldn't add an index yet. The request assumes the database is the bottleneck,
but we haven't verified that. First I'd inspect the slow query path and query plan.
If the database is the problem, an index may help. If the time is in N+1 queries
or response construction, the index won't fix it."
```

### Example 2: Don't Mistake a Symptom Patch for a Root-Cause Fix

**User Request:** "Just add retries to fix the flaky inventory sync job"

**❌ What LLMs Do (Treat the Workaround as the Solution)**
...
**Problems:**
- Assumed transient failure without evidence
- Might hide a deterministic bug behind retries
- Didn't check whether the failures are data-dependent or order-dependent
- Adds noise to operations while leaving the real defect in place

**✅ What Should Happen (Interrogate the Failure Mode)**

```
Before adding retries, find out what "flaky" means:

1. Inspect the failing runs:
   - same exception every time?
   - same input payload?
   - same step in the sync pipeline?
2. Separate transient failures from deterministic ones:
   - network timeout / 503 -> retries may help
   - duplicate key / bad state / parsing bug -> retries won't help
3. Check whether the job is non-idempotent
4. Reproduce the failure with logs or a focused test

A safer response is:

"I wouldn't add retries yet. Retries are appropriate for confirmed transient
failures, but they can also mask deterministic bugs and duplicate side effects.
First I'd inspect the error pattern and identify whether this is a network issue,
an idempotency problem, or a logic bug in the sync step."
```

### Example 3: Don't Treat Temporal Correlation as Proof

**User Request:** "Checkout started failing after yesterday's serializer refactor. Revert it."

**❌ What LLMs Do (Confuse Timing with Causality)**

```diff
-    serializer = OrderSerializerV2()
+    serializer = OrderSerializerV1()
```

**Problems:**
- Assumed the most recent visible change caused the incident
- Didn't verify whether the failing path actually goes through the serializer
- Might undo unrelated improvements while leaving the real issue unresolved
- Ignored other changes shipped at the same time, such as config, schema, or dependency updates

**✅ What Should Happen (Verify Causality Before Reverting)**

```
Before reverting, confirm that the refactor is actually part of the failing path:

1. Check the error logs and stack trace for the failing checkout request
2. Confirm whether the request reaches the serializer code at all
3. Compare the deploy timeline with config, schema, and dependency changes
4. Reproduce in staging or bisect if the cause is still unclear

A safer response is:

"Reverting may be correct, but 'started after X' is not enough evidence by itself.
First I'd verify whether the failing checkout path actually depends on the serializer
refactor and whether any config or schema changes shipped alongside it. If we need a
fast mitigation, use a narrow rollback or feature flag while we confirm causality."
```
...
|-----------|-------------|-----|
| Think Critically | Accepts the user's diagnosis and starts coding the proposed fix | Verify the premise first and confirm the real bottleneck or failure mode |
